...
final class ReferenceScraper implements Closeable {
	...
	private boolean scrapNextReference() throws IOException {
		...
		while (this.references.isEmpty() && (line = readLine()) != null) {
			...
			if (instructionEntryMatcher.matches()) {
				String instruction = decodeHtml(instructionEntryMatcher.group(1));
				...
				while (true) {
					...
					if (sectionMatcher.matches()) {
						String section = decodeHtml(sectionMatcher.group(1));
	...
	private InstructionFormat scrapeFormat() throws IOException {
		...
		String instruction = decodeHtml(formatMatcher.group(1));
		List<String> arguments = new ArrayList<>();

		formatMatcher = FORMAT_PATTERN.matcher(safeReadLine());
		while (formatMatcher.matches()) {
			arguments.add(decodeHtml(formatMatcher.group(1)));
			formatMatcher = FORMAT_PATTERN.matcher(safeReadLine());
		}
	...
	private String scrapeOperandStackIn() throws IOException {
		...
		return decodeText(operandStackInMatcher.group(1), DECODE_OPERAND_STACK_TABLE).trim();
	}
	...
	private String scrapeOperandStackOut() throws IOException {
		...
		if (operandStackOutMatcher.groupCount() != 0) {
			out = decodeText(operandStackOutMatcher.group(1), DECODE_OPERAND_STACK_TABLE).trim();
		} else {
			out = "";
		}
		return out;
